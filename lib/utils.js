...
export const anilistFetcher = async (params = "recent-episodes") => {
  const url = `${process.env.NEXT_PUBLIC_BASE_API}/anime/gogoanime`;

  try {
    const response = await fetch(`${url}/${params}`,{ next: { revalidate:10 } });
